...
const buildReceiptDoc = (order: Order, generatedByUsername: string): jsPDF => {
    ...
    doc.text('Insophinia Solutions Pvt Ltd', 14, 22);
    doc.setFontSize(10);
    ...
    doc.text(`Contact: ${order.customerContact}`, 14, 60);
    ...
    const tableData = order.items.map(item => [
        ...
        item.warrantyPeriod ? `${item.warrantyPeriod} months` : 'N/A',
    ...
    if (order.discountAmount > 0) {
        doc.setFont('helvetica', 'bold');
        const discountText = order.discountApplied > 0 ? `Discount (${order.discountApplied}%):` : 'Total Discount:';
